...
def initialize_models(opt):
    ...
    return detect_model, plate_rec_model, device


def process_images(opt, detect_model, plate_rec_model, device):
...
def process_video(opt, detect_model, plate_rec_model, device):
    save_path = opt.output
...
if __name__ == '__main__':
    # 导入其他方法，包括 parse_arguments、initialize_models 和 process_images
    imgs = ""
    # 解析命令行参数
    args = ['--detect_model', 'weights/plate_detect.pt',
    ...
    # 初始化模型
    # detect_model, plate_rec_model, device = initialize_models(opt)
    #
    # # 处理图片
    # process_images(opt, detect_model, plate_rec_model, device)


def open_file():
    file_path = filedialog.askopenfilename(
        filetypes=[("Image files", "*.jpg;*.jpeg;*.png;*.gif"), ("Video files", "*.mp4;*.avi")])
    ...
    if file_path:
        ...
        if file_path:
            ...
            if file_extension in ["jpg", "jpeg", "png", "gif"]:
                opt.image_path = str(file_path)
            elif file_extension in ["mp4", "avi"]:
                opt.video = str(file_path)
...
def process_image():
    ...
    if not opt.video:  # 处理图片
        if not os.path.isfile(opt.image_path):  # 目录
            ...
            display_image(original_image, processed_image)
        ...
        if capture.isOpened():
            while True:
                ...
                cv2.putText(ori_img, str_fps, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.imshow("Result", ori_img)  # 实时显示处理结果

                out.write(ori_img)

                key = cv2.waitKey(1)  # 等待按键输入，参数表示每帧等待时间，单位为毫秒
...
def display_image(original_image, processed_image, width=500, height=300):
    # 显示原始图片
    original_img = Image.open(original_image)
    original_img = original_img.resize((width, height))
    original_image_tk = ImageTk.PhotoImage(original_img)
    ...
    processed_image_tk = ImageTk.PhotoImage(processed_img)
...
open_button.grid(row=2, column=0, columnspan=2, pady=(0, 10))  # 将按钮放置在第三行，底部居中

# 添加一个空的行，使得打开文件按钮位于底部
...
processed_image_label.grid(row=0, column=1, padx=10, pady=10)
