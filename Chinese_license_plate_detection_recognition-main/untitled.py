...
def draw_result(orgimg, dict_list, is_color=False):  # 车牌结果画出来
    ...
    for result in dict_list:
        ...
        cof = result['detect_conf']
        cof = round(float(cof), 3)
        ...
        labelSize = cv2.getTextSize(result_p + str(cof), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)  # 获得字体的大小
        ...
        if len(result) >= 1:
            print(result_p + str(cof))
            # data_text.insert(tk.END, result_p+str(cof))
            orgimg = cv2ImgAddText(orgimg, result_p + str(cof), rect_area[0],
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, Form):
        ...
        Form.resize(1052, 663)
        self.display_label = QtWidgets.QLabel(Form)
        self.display_label.setGeometry(QtCore.QRect(20, 10, 1021, 81))

        self.display_label.setObjectName("display_label")
        self.openfile_button = QtWidgets.QPushButton(Form)
        self.openfile_button.setGeometry(QtCore.QRect(60, 120, 93, 28))
        self.openfile_button.setObjectName("openfile_button")
        self.detect_button = QtWidgets.QPushButton(Form)
        self.detect_button.setGeometry(QtCore.QRect(60, 160, 93, 28))
        self.detect_button.setObjectName("detect_button")
        self.detect_label = QtWidgets.QLabel(Form)
        self.detect_label.setGeometry(QtCore.QRect(240, 90, 781, 541))
        self.detect_label.setText("")
        self.detect_label.setObjectName("detect_label")
        self.resultedit = QtWidgets.QTextEdit(Form)
        self.resultedit.setGeometry(QtCore.QRect(30, 320, 181, 121))
        self.resultedit.setReadOnly(True)
        self.resultedit.setObjectName("resultedit")
        self.resultedit.setText("a")
        self.label_3 = QtWidgets.QLabel(Form)
        self.label_3.setGeometry(QtCore.QRect(30, 280, 111, 21))
    ...
    def retranslateUi(self, Form):
        ...
        self.display_label.setText(_translate("Form",
                                              "<html><head/><body><p align=\"center\"><span style=\" font-size:18pt; font-weight:600;\">车牌识别系统</span></p></body></html>"))
        self.openfile_button.setText(_translate("Form", "打开文件"))
        self.detect_button.setText(_translate("Form", "开始检测"))
        self.openfile_button.clicked.connect(self.openImage)
        self.detect_button.clicked.connect(self.detect)
        self.label_3.setText(_translate("Form",
                                        "<html><head/><body><p><span style=\" font-size:12pt;\">识别结果：</span></p></body></html>"))

    def openImage(self):
        imagePath, _ = QFileDialog.getOpenFileName(self, '选择图片', '', 'Images (*.png *.xpm *.jpg *.bmp *.mp4)')
        _, ext = os.path.splitext(imagePath)
        if ext == '.jpg' or ext == '.png' or ext == '.bmp':
            pixmap = QPixmap(imagePath)
            self.detect_label.setPixmap(pixmap.scaled(self.detect_label.size(), Qt.KeepAspectRatio))
            self.path = imagePath
        else:
    ...
    def update_frame(self):
        ...
        # if ret:
        #     gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # 将彩色帧转换为灰度帧
        #     self.display_frame(gray_frame)
        # else:
        #     self.timer.stop()
        ...
        if capture.isOpened():
            while True:
                ...
                # if channel == 1:
                #     img_np = np.repeat(ori_img[:, :, np.newaxis], 3, axis=2)
                # bytes_per_line = 3 * width
                # q_img = QImage(ori_img.data, width, height, bytes_per_line, QImage.Format_RGB888)
    ...
    def detect(self):
        ...
        print(opt.video)
        if not opt.video:  # 处理图片
            if not os.path.isfile(self.path):  # 目录
                file_list = []
                allFilePath(opt.image_path, file_list)  # 将这个目录下的所有图片文件路径读取到file_list里面
                for img_path in file_list:  # 遍历图片文件
                ...
                for result in dict_list:
                    P=result['plate_no']
                    C=result['plate_color']
                    print(P)
                    self.resultedit.setText(P+C)
                # print(str(dict_list['plate_no']))
                ori_img = draw_result(img, dict_list)
